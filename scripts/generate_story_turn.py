...
def build_messages(packet: dict, episode_ctx: dict, episode_target: int = 10):
    ...
    system = (
        ...
        'Стилевые ограничения: избегай шаблона противопоставления вида "это не X, это Y"; не повторяй одну синтаксическую конструкцию в соседних фразах; '
        'не допускай цепочек лексических повторов в коротком фрагменте; не злоупотребляй тире там, где естественнее запятая или обычная связка; '
        'не ставь кавычки вокруг обычных слов без необходимости; не используй рубленый телеграфный ритм из 2-3+ коротких однотипных фраз подряд. '
